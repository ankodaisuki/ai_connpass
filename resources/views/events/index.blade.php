                        ...
                        <div class="flex flex-1 flex-col p-5">
                            ...
                            <div class="mt-auto space-y-2 text-sm text-slate-600 dark:text-slate-400">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span>{{ $event->event_date->format('Y/m/d') }}（{{ $event->event_date->locale('ja')->isoFormat('ddd') }}）{{ $event->event_date->format('H:i') }}〜{{ $event->end_date->isSameDay($event->event_date) ? $event->end_date->format('H:i') : $event->end_date->format('m/d H:i') }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <svg class="h-4 w-4 flex-shrink-0 text-slate-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
